...
secret_keys = [ecdsa.SigningKey.from_string(bytes.fromhex(sk), curve=ecdsa.SECP256k1, hashfunc=sha256) for sk in secret_keys_hex]
...
for (sk, pk_hex) in zip(secret_keys, public_keys_hex):
    pk = ecdsa.VerifyingKey.from_string(bytes.fromhex(pk_hex), curve=ecdsa.SECP256k1, hashfunc=sha256)
...
for i in sig_indices:
    ...
    pk = ecdsa.VerifyingKey.from_string(bytes.fromhex(public_keys_hex[index]), curve=ecdsa.SECP256k1, hashfunc=sha256)
