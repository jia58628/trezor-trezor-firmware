...
const HOMESCREEN_IMAGE_SIZE: i16 = WIDTH / 2;
...
fn homscreen_line(
    ...
    for x in 0..HOMESCREEN_IMAGE_SIZE {
        ...
        img_buffer.buffer[2 * x0 + 1] = c0.hi_byte();
        img_buffer.buffer[2 * x0] = c0.lo_byte();
        img_buffer.buffer[2 * x1 + 1] = c1.hi_byte();
        img_buffer.buffer[2 * x1] = c1.lo_byte();
    }
...
pub fn homescreen(
    ...
    if let Some((size, format)) = toif {
        ...
        for y in 0..HOMESCREEN_IMAGE_SIZE {
            ...
            for i in 0..=1 {
                let done =
                    homscreen_line(&mut icon_data, text_buffer, text_info, &mut dest, 2 * y + i);
