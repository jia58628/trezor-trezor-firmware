...
def authorization_fields(auth: EosAuthorization) -> list[PropertyType]:
    ...
    for i, key in enumerate(auth.keys, 1):
        ...
        fields_append((header, _key))
        fields_append((w_header, _weight))
    ...
    for i, account in enumerate(auth.accounts, 1):
        ...
        fields_append((a_header, _account))
        fields_append((p_header, _permission))
        fields_append((w_header, str(account.weight)))
    ...
    for i, wait in enumerate(auth.waits, 1):
        ...
        fields_append((header, _wait + " sec"))
        fields_append((w_header, _weight))
