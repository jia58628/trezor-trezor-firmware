...
def busy_expiry_ms() -> int:
    ...
    busy_deadline_bytes = storage_cache.get(storage_cache.APP_COMMON_BUSY_DEADLINE_MS)
...
async def handle_SetBusy(ctx: wire.Context, msg: SetBusy) -> Success:
    ...
    if msg.expiry_ms:
        ...
        storage_cache.set(
            storage_cache.APP_COMMON_BUSY_DEADLINE_MS, deadline.to_bytes(4, "big")
        )
    else:
