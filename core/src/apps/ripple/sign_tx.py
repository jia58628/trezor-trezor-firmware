...
async def sign_tx(
    ...
    if msg_payment.destination_tag is not None:
        await layout.require_confirm_destination_tag(ctx, msg_payment.destination_tag)
    await layout.require_confirm_fee(ctx, msg.fee)
    await layout.require_confirm_tx(ctx, msg_payment.destination, msg_payment.amount)
