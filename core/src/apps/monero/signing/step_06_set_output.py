...
def _validate(
    ...
    _ensure(not dst_entr or dst_entr.amount >= 0, "Destination with negative amount")
    _ensure(
        state.current_input_index + 1 == state.input_count, "Invalid number of inputs"
    )
    _ensure(state.current_output_index < state.output_count, "Invalid output index")
    ...
    if not state.is_processing_offloaded:
        ...
        _ensure(crypto.ct_equals(dst_entr_hmac, dst_entr_hmac_computed), "HMAC failed")
...
def _range_proof(
    ...
    if not state_rsig_offload and provided_rsig:
        raise Error("Provided unexpected rsig")

    # Batching & validation
    bidx = _get_rsig_batch(state, state.current_output_index)
    last_in_batch = _is_last_in_batch(state, state.current_output_index, bidx)
    if state_rsig_offload and provided_rsig and not last_in_batch:
    ...
    if (
        ...
        and state_is_processing_offloaded
    ):
        raise Error("Rsig expected, not provided")

    # Batch not finished, skip range sig generation now
    mask = state.output_masks[-1] if not state_is_processing_offloaded else None
    offload_mask = mask and state_rsig_offload
    ...
    if not state_rsig_offload:
        # Bulletproof calculation in Trezor
        rsig = _rsig_bp(state)

    elif not state_is_processing_offloaded:
    ...
    if state.current_output_index + 1 == state.output_count and (
        not state_rsig_offload or state_is_processing_offloaded
    ):
...
def _dump_rsig_bp_plus(rsig: BulletproofPlus) -> bytes:
    ...
    for num, var in (
        ...
        utils.memcpy(buff, num, var, 0, 32)
