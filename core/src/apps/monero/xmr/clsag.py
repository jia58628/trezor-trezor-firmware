...
def _generate_clsag(
    ...
    sc1 = crypto_Scalar()  # sig.c1
    a = crypto.random_scalar()
    H = crypto_Point()
    D = crypto_Point()
    Cout_bf = crypto_helpers.encodepoint(Cout)

    tmp_sc = crypto_Scalar()
    tmp = crypto_Point()
    tmp_bf = bytearray(32)

    crypto.hash_to_point_into(H, P[index])
    crypto_scalarmult_into(sI, H, p)  # I = p*H
    crypto_scalarmult_into(D, H, z)  # D = z*H
    crypto_sc_mul_into(tmp_sc, z, crypto_helpers.INV_EIGHT_SC)  # 1/8*z
    crypto_scalarmult_into(sD, H, tmp_sc)  # sig.D = 1/8*z*H
    ...
    hsh_PC(crypto_encodepoint_into(tmp_bf, sI))
    hsh_PC(sD)
    hsh_PC(Cout_bf)
    ...
    for i in range(len(P)):
        c_to_hash_update(C_nonzero[i])
    c_to_hash_update(Cout_bf)
    c_to_hash_update(message)

    chasher = c_to_hash.copy()
    crypto.scalarmult_base_into(tmp, a)
    chasher.update(crypto_encodepoint_into(tmp_bf, tmp))  # aG
    crypto_scalarmult_into(tmp, H, a)
    chasher.update(crypto_encodepoint_into(tmp_bf, tmp))  # aH
    ...
    while i != index:
        ...
        crypto_sc_mul_into(c_p, mu_P, c)
        crypto_sc_mul_into(c_c, mu_C, c)
        ...
        crypto_scalarmult_into(tmp, tmp, c_c)
        ...
        crypto.point_add_into(R, R, crypto_scalarmult_into(tmp, D, c_c))

        chasher = c_to_hash.copy()
        chasher.update(crypto_encodepoint_into(tmp_bf, L))
        chasher.update(crypto_encodepoint_into(tmp_bf, R))
    ...
    crypto_sc_mul_into(tmp_sc, mu_P, p)
