...
def compute_hash(rr: MoneroTransferDetails) -> bytes:
    ...
    if rr.additional_tx_pub_keys:
        for x in rr.additional_tx_pub_keys:
            kck_update(x)
    kck_update(dump_uvarint_b(rr.internal_output_index))
    return kck.digest()
...
def export_key_image(
    ...
    if len(td_additional_tx_pub_keys) == 1:  # compression
        additional_tx_pub_key = decodepoint(td_additional_tx_pub_keys[0])
    elif td_additional_tx_pub_keys:
        if td.internal_output_index >= len(td_additional_tx_pub_keys):
            raise ValueError("Wrong number of additional derivations")
        additional_tx_pub_key = decodepoint(
            td_additional_tx_pub_keys[td.internal_output_index]
        )
