...
_tmp_pt_1 = crypto_Point()
_tmp_pt_2 = crypto_Point()
_tmp_pt_3 = crypto_Point()
_tmp_pt_4 = crypto_Point()

_tmp_sc_1 = crypto_Scalar()
_tmp_sc_2 = crypto_Scalar()
_tmp_sc_3 = crypto_Scalar()
_tmp_sc_4 = crypto_Scalar()
_tmp_sc_5 = crypto_Scalar()
...
def _load_scalar(dst: crypto.Scalar | None, a: ScalarDst) -> crypto.Scalar:
    return (
        crypto_sc_copy(dst, a)
        if isinstance(a, crypto_Scalar)
        else crypto_decodeint_into_noreduce(dst, a)
    )
...
def _invert(dst: bytearray | None, x: bytes) -> bytearray:
    ...
    crypto_encodeint_into(dst, _tmp_sc_2)
    return dst
...
def _scalarmult8(dst: bytearray | None, P, tmp_pt: crypto.Point = _tmp_pt_1):
    dst = _ensure_dst_key(dst)
    crypto_decodepoint_into(tmp_pt, P)
    crypto.ge25519_mul8(tmp_pt, tmp_pt)
    crypto_encodepoint_into(dst, tmp_pt)
    return dst


def _scalarmultH(dst: bytearray, x: bytes) -> bytearray:
    dst = _ensure_dst_key(dst)
    crypto_decodeint_into(_tmp_sc_1, x)
    crypto.scalarmult_into(_tmp_pt_1, _XMR_HP, _tmp_sc_1)
    crypto_encodepoint_into(dst, _tmp_pt_1)
    return dst
...
def _sc_gen(dst: bytearray | None = None) -> bytearray:
    ...
    crypto_encodeint_into(dst, _tmp_sc_1)
    return dst
...
def _sc_add(dst: bytearray | None, a: bytes, b: bytes) -> bytearray:
    ...
    crypto_encodeint_into(dst, _tmp_sc_3)
    return dst
...
def _sc_muladd(
    ...
    if isinstance(dst, crypto_Scalar):
        dst_sc = dst
    else:
        dst_sc = _tmp_sc_4
    if not isinstance(a, crypto_Scalar):
        crypto_decodeint_into_noreduce(_tmp_sc_1, a)
        a = _tmp_sc_1
    if not isinstance(b, crypto_Scalar):
        crypto_decodeint_into_noreduce(_tmp_sc_2, b)
        b = _tmp_sc_2
    if not isinstance(c, crypto_Scalar):
        crypto_decodeint_into_noreduce(_tmp_sc_3, c)
        c = _tmp_sc_3
    crypto.sc_muladd_into(dst_sc, a, b, c)
    if not isinstance(dst, crypto_Scalar):
        dst = _ensure_dst_key(dst)
        crypto_encodeint_into(dst, dst_sc)
    return dst
...
def _hash_vct_to_scalar(dst, data):
    ...
    crypto_decodeint_into(_tmp_sc_1, hsh)
    crypto_encodeint_into(dst, _tmp_sc_1)
    return dst
...
def _get_exponent_univ(dst, base, idx, salt):
    ...
    crypto_encodepoint_into(dst, _tmp_pt_4)
    return dst
...
def _sc_square_mult(
    ...
    if n == 0:
        return crypto_decodeint_into_noreduce(dst, _ONE)

    lg = int(math.log(n, 2))
    dst = crypto_sc_copy(dst, x)
    for i in range(1, lg + 1):
        crypto_sc_mul_into(dst, dst, dst)
        if n & (1 << (lg - i)) > 0:
            crypto_sc_mul_into(dst, dst, x)
    return dst
...
def _invert_batch(x):
    ...
    for n in range(len(x)):
        utils_ensure(x[n] != _ZERO, "cannot invert zero")
        scratch[n] = acc
        if n == 0:
...
def _sum_of_even_powers(res, x, n):
    ...
    """
    utils_ensure(n & (n - 1) == 0, "n is not 2^x")
    utils_ensure(n != 0, "n == 0")
...
class KeyV(KeyVBaseType[T]):
    ...
    def to(self, idx, buff: bytearray | None = None, offset: int = 0):
        ...
        if self.chunked:
            ...
            assert isinstance(self_d, (bytes, bytearray))
            memcpy(buff if buff else self.cur, offset, self_d, idx << 5, 32)
    ...
    def read(self, idx: int, buff: bytes, offset: int = 0) -> bytes:
        ...
        if self.chunked:
            assert isinstance(self_d, list)
            memcpy(self_d[idx >> _CHBITS], (idx & (_CHSIZE - 1)) << 5, buff, offset, 32)
        else:
            assert isinstance(self_d, bytearray)
            memcpy(self_d, idx << 5, buff, offset, 32)
...
class KeyVEval(KeyVBase):
    ...
    def __init__(self, elems=64, src=None, raw: int = False, scalar=True):
        ...
        self.buff = (
            _ensure_dst_key()
            if not raw
            else (crypto_Scalar() if scalar else crypto_Point())
        )
    ...
    def to(self, idx, buff: bytearray | None = None, offset: int = 0):
        ...
        if self.raw:
            ...
            if self.scalar and buff:
                return crypto_sc_copy(buff, self_buff)
            elif self.scalar:
                return self_buff
            else:
                raise ValueError("Not supported")
...
class KeyVPowers(KeyVBase):
    ...
    def __init__(self, size, x, raw: int = False):
        super().__init__(size)
        self.x = x if not raw else crypto_decodeint_into_noreduce(None, x)
        self.raw = raw
        self.cur = bytearray(32) if not raw else crypto_Scalar()
        self.last_idx = 0
    ...
    def __getitem__(self, item):
        ...
        if item == 0:
            return (
                _copy_key(self_cur, _ONE)
                if not self.raw
                else crypto_decodeint_into_noreduce(None, _ONE)
            )
        elif item == 1:
            return (
                _copy_key(self_cur, self_x)
                if not self.raw
                else crypto_sc_copy(self_cur, self_x)
            )
        elif item == prev:
            return self_cur
        elif item == prev + 1:
            return (
                _sc_mul(self_cur, self_cur, self_x)
                if not self.raw
                else crypto_sc_mul_into(self_cur, self_cur, self_x)
            )
        else:
    ...
    def set_state(self, idx: int, val):
        self.last_idx = idx
        if self.raw:
            return crypto_sc_copy(self.cur, val)
        else:
...
class KeyVPowersBackwards(KeyVBase):
    ...
    def __init__(
        ...
        self.x_inv = crypto_Scalar()
        self.x_max = crypto_Scalar()
        self.tmp_sc = crypto_Scalar()  # TODO: use static helper when everything works
        if x_inv:
    ...
    def reset(self):
        ...
        crypto_sc_copy(self.cur_sc, self.x_max)
    ...
    def move_more(self, item: int, prev: int):
        ...
        crypto_sc_mul_into(self.cur_sc, self.cur_sc, self.tmp_sc)
    ...
    def __getitem__(self, item):
        ...
        if item == 0:
            ...
            crypto_sc_copy(self_cur_sc, self.x)
            ...
            crypto_sc_mul_into(self_cur_sc, self_cur_sc, self.x_inv)
        ...
        return self_cur_sc if self.raw else crypto_encodeint_into(self.cur, self_cur_sc)
...
class VctD(KeyVBase):
    ...
    def __init__(self, N: int, M: int, z_sq: bytearray, raw: bool = False):
        ...
        self.N = N
        self.raw = raw
        self.z_sq = crypto_decodeint_into_noreduce(None, z_sq)
        self.z_last = crypto_Scalar()
        self.two = crypto_decodeint_into_noreduce(None, _TWO)
        self.cur_sc = crypto_Scalar()
        self.tmp_sc = crypto_Scalar()  # TODO: use static helper when everything works
    ...
    def reset(self):
        self.current_idx = 0
        self.current_n = 0
        crypto_sc_copy(self.z_last, self.z_sq)
        crypto_sc_copy(self.cur_sc, self.z_sq)
        if not self.raw:
            crypto_encodeint_into(self.cur, self.cur_sc)  # z**2 + 2**0
    ...
    def move_one(self, item: int):
        ...
        if item != 0 and self.current_n >= self.N:  # reset 2**i part,
            self.current_n = 0
            crypto_sc_mul_into(self.z_last, self.z_last, self.z_sq)
            crypto_sc_copy(self_cur_sc, self.z_last)
        else:
            crypto_sc_mul_into(self_cur_sc, self_cur_sc, self.two)
        if not self.raw:
            crypto_encodeint_into(self.cur, self_cur_sc)
    ...
    def move_more(self, item: int, prev: int):
        ...
        z_squares_to_mul = (item // self_N) - (prev // self_N)

        # If z component needs to be updated, compute update and add it
        if z_squares_to_mul > 0:
            _sc_square_mult(self_tmp_sc, self.z_sq, z_squares_to_mul)
            crypto_sc_mul_into(self.z_last, self.z_last, self_tmp_sc)
            if same_2:
                crypto_sc_mul_into(self_cur_sc, self_cur_sc, self_tmp_sc)
                return
        ...
        if not same_2:
            crypto_sc_copy(self_cur_sc, self.z_last)
            _sc_square_mult(self_tmp_sc, self.two, item % self_N)
            crypto_sc_mul_into(self_cur_sc, self_cur_sc, self_tmp_sc)
...
class KeyHadamardFoldedVct(KeyVBase):
    ...
    def __init__(
        ...
        self.cur_pt = crypto_Point()
        self.tmp_pt = crypto_Point()
        self.cur = _ensure_dst_key() if not self.raw else None
    ...
    def __getitem__(self, item):
        ...
        crypto_decodepoint_into(self.tmp_pt, self.src.to(self.size + i))
        crypto.add_keys3_into(self_cur_pt, self.a, self_cur_pt, self.b, self.tmp_pt)
        if self.gc_fnc:
            self.gc_fnc(i)
        if not self.raw:
            return crypto_encodepoint_into(self.cur, self_cur_pt)
        else:
            return self_cur_pt
...
class KeyScalarFoldedVct(KeyVBase):
    ...
    def __getitem__(self, item):
        ...
        crypto_decodeint_into_noreduce(self_tmp_sc, self.src.to(i))
        crypto_sc_mul_into(self_tmp_sc, self_tmp_sc, self.a)
        crypto_decodeint_into_noreduce(self_cur_sc, self.src.to(self.size + i))
        crypto.sc_muladd_into(self_cur_sc, self_cur_sc, self.b, self_tmp_sc)

        if self.gc_fnc:
            self.gc_fnc(i)
        if not self.raw:
            return crypto_encodeint_into(self.cur, self_cur_sc)
        else:
            return self_cur_sc
...
class KeyPow2Vct(KeyVBase):
    ...
    def __getitem__(self, item):
        ...
        if i == 0:
            ...
            _copy_key(self_cur, _TWO)
        else:
            _copy_key(self_cur, _ZERO)
            self_cur[i >> 3] = 1 << (i & 7)

        if i < 252 and self.raw:
            return crypto_decodeint_into_noreduce(self_cur_sc, self_cur)

        if i > 252:  # reduction, costly
            crypto_decodeint_into(self_cur_sc, self_cur)
            if not self.raw:
                return crypto_encodeint_into(self_cur, self_cur_sc)

        return self_cur_sc if self.raw else self_cur
...
class KeyChallengeCacheVct(KeyVBase):
    ...
    def __getitem__(self, item):
        ...
        if self.precomp:
            _copy_key(self_cur, self.precomp[i >> (self.nbits - self.precomp_depth)])
        ...
        for j in range(self.nbits - 1, bits_done - 1, -1):
            if i & (1 << (self.nbits - 1 - j)) > 0:
                _sc_mul(self_cur, self_cur, self.ch_[j])
            else:
                _sc_mul(self_cur, self_cur, self.chi[j])
        return self_cur
...
class KeyR0(KeyVBase):
    ...
    def __init__(self, size, N, aR, y, z, raw: int = False, **kwargs) -> None:
        ...
        self.aR = aR
        self.raw = raw
        self.y = crypto_decodeint_into_noreduce(None, y)
        self.yp = crypto_Scalar()  # y^{i}
        self.z = crypto_decodeint_into_noreduce(None, z)
        self.zt = crypto_Scalar()  # z^{2 + \floor{i/N}}
        self.p2 = crypto_Scalar()  # 2^{i \% N}
        self.res = crypto_Scalar()  # tmp_sc_1
    ...
    def reset(self) -> None:
        crypto_decodeint_into_noreduce(self.yp, _ONE)
        crypto_decodeint_into_noreduce(self.p2, _ONE)
        crypto_sc_mul_into(self.zt, self.z, self.z)
    ...
    def __getitem__(self, item):
        ...
        if item == 0:  # Reset on first item access
            ...
            _crypto_sc_mul_into(self.yp, self.yp, self.y)  # ypow
            if item % self.N == 0:
                _crypto_sc_mul_into(self.zt, self.zt, self.z)  # zt
                crypto_decodeint_into_noreduce(self_p2, _ONE)  # p2 reset
            else:
                crypto_decodeint_into_noreduce(self_res, _TWO)  # p2
                _crypto_sc_mul_into(self_p2, self_p2, self_res)  # p2
...
def _vector_powers(x, n, dst=None, dynamic: int = False):
    ...
    crypto_decodeint_into_noreduce(_tmp_sc_1, x)
    crypto_decodeint_into_noreduce(_tmp_sc_2, x)
    for i in range(2, n):
        crypto_sc_mul_into(_tmp_sc_1, _tmp_sc_1, _tmp_sc_2)
        crypto_encodeint_into(_tmp_bf_0, _tmp_sc_1)
...
def _cross_inner_product(l0, r0, l1, r1):
    ...
    return crypto_helpers_encodeint(sc_t1), crypto_helpers_encodeint(sc_t2)
...
def _hash_cache_mash(dst, hash_cache, *args):
    ...
    crypto_decodeint_into(_tmp_sc_1, hsh)
    crypto_encodeint_into(hash_cache, _tmp_sc_1)
    _copy_key(dst, hash_cache)
    return dst


def _is_reduced(sc) -> bool:
    return crypto_encodeint_into(_tmp_bf_0, crypto_decodeint_into(_tmp_sc_1, sc)) == sc
...
class MultiExpSequential:
    ...
    def _acc(self, scalar, point) -> None:
        crypto_decodeint_into_noreduce(_tmp_sc_1, scalar)
        crypto_decodepoint_into(_tmp_pt_2, point)
    ...
    def eval(self, dst):
        dst = _ensure_dst_key(dst)
        return crypto_encodepoint_into(dst, self.acc)
...
class BulletProofBuilder:
    ...
    def _det_mask(self, i, is_sL: bool = True, dst: bytearray | None = None):
        ...
        crypto_encodeint_into(dst, self.tmp_sc_1)
        return dst
    ...
    def sX_gen(self, ln=_BP_N) -> KeyV:
        ...
        for i in range(ln):
            ...
            crypto_encodeint_into(buff_mv[i * 32 : (i + 1) * 32], sc)
            _gc_iter(i)
    ...
    def prove_setup(self, sv: list[crypto.Scalar], gamma: list[crypto.Scalar]) -> tuple:
        ...
        utils_ensure(len(sv) == len(gamma), "|sv| != |gamma|")
        utils_ensure(len(sv) > 0, "sv empty")

        self.proof_sec = random.bytes(64)
        self._det_mask_init()
        gc_collect()
        sv = [crypto_helpers_encodeint(x) for x in sv]
        gamma = [crypto_helpers_encodeint(x) for x in gamma]
    ...
    def prove_setup_aLaR(self, MN, sv, sv_vct=None):
        sv_vct = sv_vct if sv_vct else [crypto_helpers_encodeint(x) for x in sv]
    ...
    def _prove_loop(self, MN, logMN, l, r, y, x_ip, hash_cache) -> tuple:
        ...
        yinvpowL = KeyVPowers(MN, _invert(_tmp_bf_0, y), True)
        yinvpowR = KeyVPowers(MN, _tmp_bf_0, True)
        tmp_pt = crypto_Point()
        ...
        while nprime > 1:
            ...
            _inner_product(
                aprime_slice_view(0, nprime), bprime_slice_view(nprime, npr2), cL
            )

            _inner_product(
                aprime_slice_view(nprime, npr2), bprime_slice_view(0, nprime), cR
            )
            self_gc(23)
            ...
            _vector_exponent_custom(
                ...
                aprime_slice_view(0, nprime),
                bprime_slice_view(nprime, npr2),
                _tmp_bf_0,
            )
            ...
            _vector_exponent_custom(
                ...
                aprime_slice_view(nprime, npr2),
                bprime_slice_view(0, nprime),
                _tmp_bf_0,
            )
    ...
    def verify_batch(self, proofs: list[Bulletproof], single_optim: bool = True):
        ...
        for proof in proofs:
            for cond, msg in (
                (_is_reduced(proof.taux), "Input scalar not in range"),
                (_is_reduced(proof.mu), "Input scalar not in range"),
                (_is_reduced(proof.a), "Input scalar not in range"),
                (_is_reduced(proof.b), "Input scalar not in range"),
                (_is_reduced(proof.t), "Input scalar not in range"),
...
class BulletProofPlusBuilder:
    ...
    def aX_vcts(self, sv, MN) -> tuple:
        num_inp = len(sv)
        sc_zero = crypto_decodeint_into_noreduce(None, _ZERO)
        sc_one = crypto_decodeint_into_noreduce(None, _ONE)
        sc_mone = crypto_decodeint_into_noreduce(None, _MINUS_ONE)
    ...
    def _prove_batch_main(
        ...
        while nprime > 1:
            ...
            elif (self_save_mem and rnd == 1) or (not self_save_mem and rnd == 0):
            ...
            if (self_save_mem and rnd == 1) or (not self_save_mem and rnd == 0):
                pass
                # self.aL = None
    ...
    def verify_batch(self, proofs: list[BulletproofPlus]):
        ...
        for proof in proofs:
            ...
            to_invert.read(to_invert_offset, pd_y)
            to_invert_offset += 1
            inv_offset += rounds + 1
            self_gc(2)
        ...
        for proof in proofs:
            ...
            challenges_cache[1] = pd_challenges[0]
        ...
        muex_expl_add_pair(G_scalar, _XMR_G)
        muex_expl_add_pair(H_scalar, _XMR_H)
        # for i in range(maxMN):
