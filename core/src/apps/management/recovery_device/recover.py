...
def process_slip39(words: str) -> tuple[bytes | None, slip39.Share]:
    ...
    if not remaining:
        recovery.set_slip39_group_count(share.group_count)
        recovery.set_slip39_iteration_exponent(share.iteration_exponent)
        recovery.set_slip39_identifier(share.identifier)
        recovery.set_slip39_remaining_shares(share.threshold - 1, share_group_index)
        recovery_shares.set(share.index, share_group_index, words)
    ...
    if share.identifier != recovery.get_slip39_identifier():
        raise RuntimeError("Slip39: Share identifiers do not match")
    if share.iteration_exponent != recovery.get_slip39_iteration_exponent():
        raise RuntimeError("Slip39: Share exponents do not match")
    if recovery_shares.get(share.index, share_group_index):
        raise RuntimeError("Slip39: This mnemonic was already entered")
    if share.group_count != recovery.get_slip39_group_count():
        raise RuntimeError("Slip39: Group count does not match")

    remaining_for_share = (
        recovery.get_slip39_remaining_shares(share_group_index) or share.threshold
    )
    recovery.set_slip39_remaining_shares(remaining_for_share - 1, share_group_index)
    remaining[share_group_index] = remaining_for_share - 1
    recovery_shares.set(share.index, share_group_index, words)
