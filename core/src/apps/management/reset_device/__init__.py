...
async def reset_device(ctx: Context, msg: ResetDevice) -> Success:
    ...
    if msg_backup_type == BAK_T_SLIP39_BASIC:
        prompt = "Create a new wallet\nwith Shamir Backup?"
    elif msg_backup_type == BAK_T_SLIP39_ADVANCED:
        prompt = "Create a new wallet\nwith Super Shamir?"
    else:
    ...
    if msg_backup_type == BAK_T_BIP39:
        # in BIP-39 we store mnemonic string instead of the secret
        secret = bip39.from_data(secret).encode()
    elif msg_backup_type in (BAK_T_SLIP39_BASIC, BAK_T_SLIP39_ADVANCED):
    ...
    if perform_backup:
        await backup_seed(ctx, msg_backup_type, secret)
...
def _validate_reset_device(msg: ResetDevice) -> None:
    ...
    if backup_types.is_slip39_backup_type(msg_backup_type):
