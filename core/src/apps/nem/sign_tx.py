...
async def sign_tx(ctx: Context, msg: NEMSignTx, keychain: Keychain) -> NEMSignedTx:
    ...
    await validate_path(
        ctx,
        keychain,
        msg_transaction.address_n,
        check_path(msg_transaction.address_n, msg_transaction.network),
    )

    node = keychain.derive(msg_transaction.address_n)
    ...
    if msg_multisig:
        ...
        if msg.cosigning:
            ...
            tx = multisig.initiate(
                seed.remove_ed25519_prefix(node.public_key()), msg_transaction, tx
            )
