...
def request_tx_extra_data(
    ...
    tx_req_details.extra_data_offset = offset
    tx_req_details.extra_data_len = size
    tx_req_details.tx_hash = tx_hash
...
def _clear_tx_request(tx_req: TxRequest) -> None:
    ...
    assert tx_req_serialized.serialized_tx is not None
    tx_req.request_type = None
    tx_req_details.request_index = None
    tx_req_details.tx_hash = None
    tx_req_details.extra_data_len = None
    tx_req_details.extra_data_offset = None
    tx_req_serialized.signature = None
    tx_req_serialized.signature_index = None
    # typechecker thinks serialized_tx is `bytes`, which is immutable
    # we know that it is `bytearray` in reality
    tx_req_serialized.serialized_tx[:] = bytes()  # type: ignore ["__setitem__" method not defined on type "bytes"]
...
def _sanitize_tx_input(txi: TxInput, coin: CoinInfo) -> TxInput:
    ...
    if txi.multisig and txi_script_type not in common.MULTISIG_INPUT_SCRIPT_TYPES:
        raise DataError("Multisig field provided but not expected.")

    if not txi.multisig and txi_script_type == InputScriptType.SPENDMULTISIG:
        raise DataError("Multisig details required.")

    if txi_script_type in common.INTERNAL_INPUT_SCRIPT_TYPES:
    ...
    if txi_script_type in common.SEGWIT_INPUT_SCRIPT_TYPES or txi.witness is not None:
        if not coin.segwit:
            raise DataError("Segwit not enabled on this coin.")

    if txi_script_type == InputScriptType.SPENDTAPROOT and not coin.taproot:
...
def _sanitize_tx_output(txo: TxOutput, coin: CoinInfo) -> TxOutput:
    ...
    if not txo.multisig and txo_script_type == OutputScriptType.PAYTOMULTISIG:
        raise DataError("Multisig details required.")

    if txo_address_n and txo_script_type not in common.CHANGE_OUTPUT_SCRIPT_TYPES:
        raise DataError("Output's address_n provided but not expected.")

    if txo.amount is None:
        raise DataError("Missing amount field.")

    if txo_script_type in common.SEGWIT_OUTPUT_SCRIPT_TYPES:
        if not coin.segwit:
            raise DataError("Segwit not enabled on this coin.")

    if txo_script_type == OutputScriptType.PAYTOTAPROOT and not coin.taproot:
        raise DataError("Taproot not enabled on this coin")

    if txo_script_type == OutputScriptType.PAYTOOPRETURN:
        # op_return output
        ...
        if txo.address or txo_address_n or txo.multisig:
        ...
        if txo_address_n and txo.address:
            raise DataError("Both address and address_n provided.")
        if not txo_address_n and not txo.address:
