...
async def get_ownership_proof(
    ...
    if msg_script_type not in common.INTERNAL_INPUT_SCRIPT_TYPES:
        raise DataError("Invalid script type")

    if msg_script_type in common.SEGWIT_INPUT_SCRIPT_TYPES and not coin.segwit:
        raise DataError("Segwit not enabled on this coin")

    if msg_script_type == InputScriptType.SPENDTAPROOT and not coin.taproot:
        raise DataError("Taproot not enabled on this coin")

    node = keychain.derive(msg.address_n)
    address = addresses.get_address(msg_script_type, coin, node, msg.multisig)
    ...
    if msg.multisig:
        if ownership_id not in msg_ownership_ids:
            raise DataError("Missing ownership identifier")
    elif msg_ownership_ids:
        if msg_ownership_ids != [ownership_id]:
            raise DataError("Invalid ownership identifier")
    else:
        msg_ownership_ids = [ownership_id]
