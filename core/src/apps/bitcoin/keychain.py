...
def validate_path_against_script_type(
    ...
    if script_type == InputScriptType.SPENDADDRESS and not multisig:
        patterns_append(PATTERN_BIP44)
        if coin_slip44 == _SLIP44_BITCOIN:
            patterns_append(PATTERN_GREENADDRESS_A)
            patterns_append(PATTERN_GREENADDRESS_B)

    elif (
        script_type in (InputScriptType.SPENDADDRESS, InputScriptType.SPENDMULTISIG)
        and multisig
    ):
        patterns_append(PATTERN_BIP48_RAW)
        if coin_slip44 == _SLIP44_BITCOIN or (
            coin.fork_id is not None and coin_slip44 != _SLIP44_TESTNET
        ):
            patterns_append(PATTERN_BIP45)
        if coin_slip44 == _SLIP44_BITCOIN:
            patterns_append(PATTERN_GREENADDRESS_A)
            patterns_append(PATTERN_GREENADDRESS_B)
        if coin.coin_name in BITCOIN_NAMES:
            patterns_append(PATTERN_UNCHAINED_HARDENED)
            patterns_append(PATTERN_UNCHAINED_UNHARDENED)
            patterns_append(PATTERN_UNCHAINED_DEPRECATED)

    elif coin.segwit and script_type == InputScriptType.SPENDP2SHWITNESS:
        patterns_append(PATTERN_BIP49)
        if multisig:
            patterns_append(PATTERN_BIP48_P2SHSEGWIT)
        if coin_slip44 == _SLIP44_BITCOIN:
            patterns_append(PATTERN_GREENADDRESS_A)
            patterns_append(PATTERN_GREENADDRESS_B)
        if coin.coin_name in BITCOIN_NAMES:
            patterns_append(PATTERN_CASA)

    elif coin.segwit and script_type == InputScriptType.SPENDWITNESS:
        patterns_append(PATTERN_BIP84)
        if multisig:
            patterns_append(PATTERN_BIP48_SEGWIT)
        if coin_slip44 == _SLIP44_BITCOIN:
            patterns_append(PATTERN_GREENADDRESS_A)
            patterns_append(PATTERN_GREENADDRESS_B)

    elif coin.taproot and script_type == InputScriptType.SPENDTAPROOT:
        patterns_append(PATTERN_BIP86)
        patterns_append(PATTERN_SLIP25_TAPROOT)

    return any(
        PathSchema.parse(pattern, coin_slip44).match(address_n) for pattern in patterns
    )
