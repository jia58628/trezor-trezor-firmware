...
async def _read_cmd(iface: HID) -> Cmd | None:
    ...
    while True:
        ...
        if ifrm.cmd & _TYPE_MASK == _TYPE_CONT:
            ...
            if __debug__:
                log_warning(__name__, "_TYPE_CONT")
            return None
        ...
        if bcnt > _MAX_U2FHID_MSG_PAYLOAD_LEN:
            ...
            if __debug__:
                log_warning(__name__, "_MAX_U2FHID_MSG_PAYLOAD_LEN")
        ...
        while datalen < bcnt:
            ...
            if not isinstance(buf, bytes):
                if __debug__:
                    log_warning(__name__, "_ERR_MSG_TIMEOUT")
            ...
            if cfrm.seq != seq:
                ...
                # current msg
                if __debug__:
                    log_warning(__name__, "_ERR_INVALID_SEQ")
...
class Fido2ConfirmMakeCredential(Fido2State, ConfirmInfo):
    ...
    async def on_confirm(self) -> None:
        ...
        cmd = Cmd(self_cid, _CMD_CBOR, bytes([_ERR_NONE]) + response_data)
        if self._resident:
            send_cmd_sync(
                cmd_keepalive(self_cid, _KEEPALIVE_STATUS_PROCESSING), self.iface
            )
            if not store_resident_credential(self._cred):
                cmd = cbor_error(self_cid, _ERR_KEY_STORE_FULL)
        await send_cmd(cmd, self.iface)
        self.finished = True
...
class Fido2ConfirmGetAssertion(Fido2State, ConfirmInfo, Pageable):
    ...
    async def on_confirm(self) -> None:
        ...
        try:
            ...
            cmd = cbor_error(self_cid, e.code)
        except KeyError:
            cmd = cbor_error(self_cid, _ERR_MISSING_PARAMETER)
        except Exception as e:
            # Firmware error.
            if __debug__:
                log.exception(__name__, e)
            cmd = cbor_error(self_cid, _ERR_OTHER)
...
class DialogManager:
    ...
    async def keepalive_loop(self) -> None:
        ...
        try:
            ...
            while utime.ticks_ms() < self.deadline:
                if self_state.keepalive_status() != _KEEPALIVE_STATUS_NONE:
                    cmd = cmd_keepalive(self_state.cid, self_state.keepalive_status())
                    await send_cmd(cmd, self.iface)
    ...
    async def dialog_workflow(self) -> None:
        ...
        try:
            ...
            elif self_result == _RESULT_CANCEL:
                await self_state.on_cancel()
            elif self_result == _RESULT_TIMEOUT:
                await self_state.on_timeout()
            else:
                await self_state.on_decline()
...
def _dispatch_cmd(req: Cmd, dialog_mgr: DialogManager) -> Cmd | None:
    ...
    if req_cmd == _CMD_MSG:
        ...
        if m.cla != 0:
            if __debug__:
                log_warning(__name__, "_SW_CLA_NOT_SUPPORTED")
            return msg_error(req_cid, _SW_CLA_NOT_SUPPORTED)

        if m.lc + _APDU_DATA > len(req.data):
            if __debug__:
                log_warning(__name__, "_SW_WRONG_LENGTH")
            return msg_error(req_cid, _SW_WRONG_LENGTH)

        if m_ins == _MSG_REGISTER:
            if __debug__:
                log_debug(__name__, "_MSG_REGISTER")
            return _msg_register(m, dialog_mgr)
        elif m_ins == _MSG_AUTHENTICATE:
            if __debug__:
                log_debug(__name__, "_MSG_AUTHENTICATE")
            return _msg_authenticate(m, dialog_mgr)
        elif m_ins == _MSG_VERSION:
            if __debug__:
                log_debug(__name__, "_MSG_VERSION")
            # msg_version
            if m.data:
            ...
            if __debug__:
                log_warning(__name__, "_SW_INS_NOT_SUPPORTED: %d", m_ins)
            return msg_error(req_cid, _SW_INS_NOT_SUPPORTED)

    elif req_cmd == _CMD_INIT:
        if __debug__:
            log_debug(__name__, "_CMD_INIT")
        return cmd_init(req)
    elif req_cmd == _CMD_PING:
        if __debug__:
            log_debug(__name__, "_CMD_PING")
        return req
    elif req_cmd == _CMD_WINK and _ALLOW_WINK:
        if __debug__:
            log_debug(__name__, "_CMD_WINK")
        ...
        if req_data_first == _CBOR_MAKE_CREDENTIAL:
            ...
            if __debug__:
                log_debug(__name__, "_CBOR_GET_ASSERTION")
            return _cbor_get_assertion(req, dialog_mgr)
        elif req_data_first == _CBOR_GET_INFO:
            if __debug__:
                log_debug(__name__, "_CBOR_GET_INFO")
            return _cbor_get_info(req)
        elif req_data_first == _CBOR_CLIENT_PIN:
            if __debug__:
                log_debug(__name__, "_CBOR_CLIENT_PIN")
            return _cbor_client_pin(req)
        elif req_data_first == _CBOR_RESET:
            if __debug__:
                log_debug(__name__, "_CBOR_RESET")
            return _cbor_reset(req, dialog_mgr)
        elif req_data_first == _CBOR_GET_NEXT_ASSERTION:
            if __debug__:
                log_debug(__name__, "_CBOR_GET_NEXT_ASSERTION")
            return cbor_error(req_cid, _ERR_NOT_ALLOWED)
        else:
            if __debug__:
                log_warning(__name__, "_ERR_INVALID_CMD _CMD_CBOR %d", req_data_first)
            return cbor_error(req_cid, _ERR_INVALID_CMD)

    elif req_cmd == _CMD_CANCEL:
        if __debug__:
            log_debug(__name__, "_CMD_CANCEL")
        dialog_mgr.result = _RESULT_CANCEL
        dialog_mgr.reset()
        return None
    else:
        if __debug__:
            log_warning(__name__, "_ERR_INVALID_CMD: %d", req_cmd)
        return cmd_error(req_cid, _ERR_INVALID_CMD)
...
def cmd_init(req: Cmd) -> Cmd:
    ...
    return Cmd(req_cid, req.cmd, bytes(buf))
...
def _msg_register(req: Msg, dialog_mgr: DialogManager) -> Cmd:
    ...
    if not storage_device.is_initialized():
        ...
        return cmd_error(req_cid, _ERR_CHANNEL_BUSY)
    ...
    chal = req_data[:32]
    cred = U2fCredential()
    cred.rp_id_hash = req_data[32:]
    cred.generate_key_handle()

    # check equality with last request
    new_state = U2fConfirmRegister(req_cid, dialog_mgr.iface, req_data, cred)
    ...
    return Cmd(req_cid, _CMD_MSG, buf)
...
def _msg_register_sign(challenge: bytes, cred: U2fCredential) -> bytes:
    ...
    sig = basic_attestation_sign((b"\x00", cred.rp_id_hash, challenge, cred_id, pubkey))

    # pack to a response
    buf, resp = make_struct(
        _resp_cmd_register(len(cred_id), len(_FIDO_ATT_CERT), len(sig))
    )
    resp.registerId = _U2F_REGISTER_ID
    utils_memcpy(resp.pubKey, 0, pubkey, 0, len(pubkey))
    resp.keyHandleLen = len(cred_id)
    utils_memcpy(resp.keyHandle, 0, cred_id, 0, len(cred_id))
    utils_memcpy(resp.cert, 0, _FIDO_ATT_CERT, 0, len(_FIDO_ATT_CERT))
    utils_memcpy(resp.sig, 0, sig, 0, len(sig))
...
def _msg_authenticate(req: Msg, dialog_mgr: DialogManager) -> Cmd:
    ...
    auth = overlay_struct(bytearray(req_data), _req_cmd_authenticate(khlen))
    ...
    if dialog_mgr.result != _RESULT_CONFIRM:
        if __debug__:
            log_info(__name__, "request declined")
        # There is no standard way to decline a U2F request, but responding with ERR_CHANNEL_BUSY
        # doesn't seem to violate the protocol and at least stops Chrome from polling.
        return cmd_error(req_cid, _ERR_CHANNEL_BUSY)
    ...
    if __debug__:
        log_info(__name__, "signing authentication")
...
def _cbor_make_credential_process(req: Cmd, dialog_mgr: DialogManager) -> State | Cmd:
    ...
    try:
        ...
        if not utils.is_empty_iterator(excluded_creds):
            ...
            return Fido2ConfirmExcluded(req_cid, dialog_mgr.iface, cred)
    ...
    if user_verification and not config.has_pin():
        ...
        return Fido2ConfirmNoPin(req_cid, dialog_mgr.iface)
...
def _cbor_get_assertion_process(req: Cmd, dialog_mgr: DialogManager) -> State | Cmd:
    ...
    if not cred_list:
        ...
        try:
            ...
            return Cmd(req_cid, _CMD_CBOR, bytes([_ERR_NONE]) + response_data)
        except Exception as e:
            # Firmware error.
            if __debug__:
                log.exception(__name__, e)
...
def _cbor_client_pin(req: Cmd) -> Cmd:
    ...
    return Cmd(req_cid, _CMD_CBOR, bytes([_ERR_NONE]) + cbor.encode(response_data))
