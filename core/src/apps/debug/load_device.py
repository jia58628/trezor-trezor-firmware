...
async def load_device(ctx: Context, msg: LoadDevice) -> Success:
    ...
    if not msg_mnemonics:
        raise ProcessError("No mnemonic provided")

    word_count = len(msg_mnemonics[0].split(" "))
    for m in msg_mnemonics[1:]:
    ...
    if not is_slip39 and not msg.skip_checksum and not bip39.check(msg_mnemonics[0]):
    ...
    if not is_slip39:  # BIP-39
        secret = msg_mnemonics[0].encode()
        backup_type = BackupType.Bip39
    else:
        identifier, iteration_exponent, secret = slip39.recover_ems(msg_mnemonics)

        # this must succeed if the recover_ems call succeeded
        share = slip39.decode_mnemonic(msg_mnemonics[0])
