...
def token_by_chain_address(chain_id: int, address: bytes) -> TokenInfo:
    symbol_decimal = _get_symbol_and_decimal(chain_id, address)
    if symbol_decimal is None:
        return UNKNOWN_TOKEN
    else:
        symbol, decimal = symbol_decimal
        return TokenInfo(symbol, decimal)


def _get_symbol_and_decimal(chain_id: int, address: bytes) -> tuple[str, int] | None:
    for t in _token_iterator(chain_id):
        if t[0] == address:
            return t[1], t[2]
    return None
