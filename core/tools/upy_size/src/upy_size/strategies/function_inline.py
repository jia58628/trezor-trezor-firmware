...
from .helpers import Function, get_all_toplevel_functions, number_of_occurrences
...
def function_inline(
    ...
    inline_candidates: list[InlineFunction] = []

    not_to_report = settings.not_inlineable_funcs

    for toplevel_func in get_all_toplevel_functions(file_content):
        ...
        if number_of_occurrences(file_content, func_regex) == 2:
            inline_candidates.append(InlineFunction(toplevel_func))

    return sorted(inline_candidates, key=lambda x: x.func.line_no)
